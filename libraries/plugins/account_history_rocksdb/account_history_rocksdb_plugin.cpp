...
public:
   ...
   void openDb()
   {
      ...
      if(status.ok())
      {
         ...
         _post_apply_op = _mainDb.post_apply_operation_proxy(
         ...
         _irreversible_block = _mainDb.irreversible_block_proxy(
            [&]( uint32_t block_num )
            {
   ...
   void shutdownDb()
   {
      chain::util::disconnect_signal(_post_apply_op);
      chain::util::disconnect_signal(_irreversible_block);
      flushStorage();
...
private:
   ...
   boost::signals2::connection      _post_apply_op;
   boost::signals2::connection      _irreversible_block;
